...
df_run_x =df[mean_names] #cbind(  df[sd_names], df[num_st_names], df[vol_st_names], df[len_st_names], df[BUAN_st_names], df[AI_st_names] ) #cbind( df[mean_names], df[sd_names], df[num_st_names], df[vol_st_names], df[len_st_names] )
all_names = c(mean_names)#c(sd_names, num_st_names, vol_st_names, len_st_names,BUAN_st_names , AI_st_names )
...
ex_cv <- cv.glmnet(df_run_x, df_run_y, alpha = 1) #
...
ex <- glmnet( x=df_run_x,y=df_run_y,alpha = 1, lambda = ex_cv$lambda.min )
...
for (j in index) {
  ...
  ggplot(as.data.frame(df_run), aes( x=as.numeric(y), y= df_run_x[,j]  ) )  +
       geom_smooth( method='lm') + geom_point()#+geom_text(x=40, y=15, label=paste0("Intercept P Values" , ) )
...
# 
# age_gap_whole = as.data.frame(predict(ex, newx = df_run_x) - df_run_y   )
